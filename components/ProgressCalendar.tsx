  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm">
      <div className="flex justify-between items-center mb-6">
        <h3 className="font-semibold text-slate-800">{format(today, 'yyyy')} оны {format(today, 'M')}-р сар</h3>
        <span className="text-xs font-medium text-slate-400 uppercase tracking-wider">Таны дараалал</span>
      </div>
      
      <div className="grid grid-cols-7 gap-2 mb-2">
        {['Н', 'Д', 'М', 'Л', 'П', 'Б', 'Б'].map((d, i) => (
          <div key={i} className="text-[10px] text-center font-bold text-slate-300">
            {d}
          </div>
        ))}
      </div>
      ...
      <div className="mt-6 pt-6 border-t border-slate-50">
        <div className="flex items-center gap-3 text-sm text-slate-600">
          ...
          <span>Идэвхтэй суралцсан өдрүүд</span>
        </div>
      </div>
    </div>
  );
